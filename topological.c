...
{
case 1:
printf("Enter the no of vertices:"); scanf("%d",&n); for(i=1;i<=n;i++)
{
q[i]=0;
visited[i]=0;
}
...
if(f<=r)
{
visited[q[f]]=1; printf("%d\t",q[f]); bfs(q[f++]);
}
}
dfs(int v)
{
...
{
if(front==-1) front=0; rear=rear+1;
queue[rear] = vertex ;
}
}
int isEmpty_queue()
...
{
del_item = queue[front]; front = front+1;
return del_item;
}
...
void find_indegree()
{
int i,j,sum; for(j=0;j<n;j++)
{
sum=0; for(i=0;i<n;i++) sum+=a[i][j]; indegree[j]=sum;
}
}
int topology()
{
...
printf("The topological sequence is:"); for(i=0;i<n;i++)
printf("\t%d",t[i]+1);
}
